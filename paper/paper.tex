...
\emph{[Numbers to be filled once the scaling fit runs on both public traces.]}

\section{Sizing Accuracy}
\label{sec:sizing}

\emph{[Held-out evaluation: last week of each trace as test set, first three
weeks as training. Baselines: (i) $\Ctop$ from log-normal fit,
(ii) 99.8th empirical percentile with 1\,000-bootstrap CI, (iii)
$\mathbb{E}[C] + 3\,\text{std}(C)$, (iv) univariate LSTM forecast at 99.8th
predicted percentile. Report coverage at the promised 99.8\% and
per-method over-provisioning cost, per VM and pooled.]}
